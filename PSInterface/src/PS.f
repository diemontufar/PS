...
c                -k conformer -R Radius -s seed -S scan -h -P process_type
c
...
c
       ...
       integer*2     is_automatic ! Results 1: Is automatic / 0: Is not automatic
       integer*2     success ! Results 1: Process ended successfully / 0: Error during execution
       integer*2     stat ! To close file if is opened unnecessary
       character*132 argument
       character*132 filename,output
       character*256 pro_output ! String containing parameters for automatic process
       character*12  pro_objective, pro_rad, pro_distance, pro_sddist ! result variables useful for automatc process
       character*6   pro_type ! input parameter (normal/auto) specified by the user 
       character*20  pro_length ! result variables useful for automatc process
       character*80  String
...
c
       ...
       pi=4.0D0*atan(1.0D0)
       pro_output = ';OK;' !Initialize output string for automatic process
       is_automatic = 0 ! Is not automatic by default
       success = 0 !Program is not successfuly executed yet
c
       arg=0
       nargs=26
c
...
c
 	 ...
 	 open(unit=7,iostat=stat,file=output,status='new')
	 out=7
	...
	elseif (argument.eq.'-P') then !Automatic process Parameter
         arg=arg+1
         call getarg(arg,argument)
         read(argument,'(A)') pro_type
		if(pro_type.eq.'auto') then
	 	   out=6 ! Force on-screen output 
		   is_automatic = 1 ! Now is automatic
		   if (stat.eq.0) close(7, status='delete')
	        endif
	 ...
	 write(6,'(A)')" pro_type  = process type (normal/auto) forces 
...
c
       if(first.gt.last) then
        write(6,'(A)')"Index of first (-f) residue is greater than las
     +t (-l)"
        write(6,'(A8,I5,A1,A8,I5,A1)')"first = ",first,first_ins,
     +                                  " last = ",last,last_ins
        write(6,'(A)')"Program must stop"
        call exit(2)
        stop
...
c
     ...
     +    (scan_type(1:6).ne.'coarse')) then
        write(6,'(A)')"Unable to read scan type"
        write(6,'(A)')"Program must stop"
        call exit(4)
        stop
       endif
...
c       write(out,'(A)')"Invoked using: ",argument
      endif !automatic
c
...
c
 ...
 4    continue
c
       if(pdbhelix.ne.0.and.is_automatic.ne.1) then !automatic process  
         write(out,'(A)')" "
...
c       Concatenate to pro_output variable for automatic process        
	write( pro_objective, '(1E12.6)' ) objective
	pro_output = trim(pro_output) // pro_objective // ";"
c       Concatenate to pro_output variable for automatic process	
	write( pro_rad, '(1F12.3)' ) param(7)
	pro_output = trim(pro_output) // pro_rad // ";"
...
c       Concatenate to pro_output variable for automatic process
	 write( pro_distance, '(1F8.3)' ) dav
	 pro_output = trim(pro_output) // pro_distance // ";"
c	 write( 6, '(A256)') pro_output
c       Concatenate to pro_output variable for automatic process
	 write( pro_sddist, '(1F5.3)' ) s2
	 pro_output = trim(pro_output) // pro_sddist // ";"
...
c	Concatenate to pro_output variable for automatic process
	 write( pro_length, '(1F8.3)' ) length
	 pro_output = trim(pro_output) // pro_length

       success = 1

       if (success.eq.1.and.is_automatic.eq.1) then
	pro_output = trim(pro_output) // ";"	
	write(6,'(A)') trim(pro_output)
	endif
